...
* `bastert`: for each variable (or at `init-random` rate) use cost values to
...
* `cycle` (default): Only for the optimization mode, start with the
  ...
  optimization fails to improve the current solution.

| init-random
| real
| [0-1] (default, 0.5) p parameter of the bernoulli's law to be used in conjunction with the `init-policy` parameter.
